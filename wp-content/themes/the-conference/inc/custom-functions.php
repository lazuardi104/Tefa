...
function the_conference_scripts() {
    ...
    if( get_theme_mod( 'ed_localgoogle_fonts',false ) && ! is_customize_preview() && ! is_admin() ){
        if ( get_theme_mod( 'ed_preload_local_fonts',false ) ) {
			the_conference_load_preload_local_fonts( the_conference_get_webfont_url( the_conference_fonts_url() ) );
        }
        wp_enqueue_style( 'the-conference-google-fonts', the_conference_get_webfont_url( the_conference_fonts_url() ) );
    }else{
