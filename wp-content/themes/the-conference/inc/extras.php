...
if( ! function_exists( 'the_conference_load_preload_local_fonts') ) :
/**
 ...
 */
function the_conference_load_preload_local_fonts( $url, $format = 'woff2' ) {

    // Check if cached font files data preset present or not. Basically avoiding 'the_conference_WebFont_Loader' class rendering.
    $local_font_files = get_site_option( 'the_conference_local_font_files', false );

    if ( is_array( $local_font_files ) && ! empty( $local_font_files ) ) {
        $font_format = apply_filters( 'the_conference_local_google_fonts_format', $format );
    ...
    $font = the_conference_webfont_loader_instance( $url );
    $font->set_font_format( $format );
    $font->preload_local_fonts();
}
endif;

if( ! function_exists( 'the_conference_flush_local_google_fonts' ) ){
    /**
     * Ajax Callback for flushing the local font
     */
    function the_conference_flush_local_google_fonts() {
        $WebFontLoader = new The_Conference_WebFont_Loader();
        //deleting the fonts folder using ajax
        $WebFontLoader->delete_fonts_folder();
        die();
    }
}
add_action( 'wp_ajax_flush_local_google_fonts', 'the_conference_flush_local_google_fonts' );
add_action( 'wp_ajax_nopriv_flush_local_google_fonts', 'the_conference_flush_local_google_fonts' );
