...
if ( ! function_exists( 'the_conference_customizer_theme_info' ) ) :
	...
	function the_conference_customizer_theme_info( $wp_customize ) {
	    ...
	    /* translators: 1: string, 2: documentation url, 3: string */
	    $theme_info .= sprintf( '%1$s<a href="%2$s" target="_blank">%3$s</a>', esc_html__( 'Documentation Link : ', 'the-conference' ), esc_url( 'https://docs.rarathemes.com/docs/the-conference/' ), esc_html__( 'Click here.', 'the-conference' ) );
	    $theme_info .= '</p>';
	...
	$manager->add_section(
		new The_Conference_Customize_Section_Pro(
			...
			array(
				...
				'pro_url'  => 'https://rarathemes.com/wordpress-themes/the-conference-pro/'
			)
		)
	);
}