...
all=$.extend({},allWaypoints.horizontal,allWaypoints.vertical);return $.map(ids,function(id){return all[id]})};return Waypoint})();methods={init:function(f,options){if(options==null){options={}}
if(options.handler==null){options.handler=f}
this.each(function(){var $this,context,contextElement,_ref;$this=$(this);contextElement=(_ref=options.context)!=null?_ref:$.fn[wp].defaults.context;if(!$.isWindow(contextElement)){contextElement=$this.closest(contextElement)}
