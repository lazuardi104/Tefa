<?php
/**
 * Influencer Customizer Notification Section Class.
 *
 * @package Influencer
 */

/**
 * Influencer_Customizer_Notice_Section class
 */
class Influencer_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $influencer_recommended_plugins as $slug => $plugin_opt ) {
			...
			$influencer_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$influencer_recommended_plugin['class'] = 'active';
			} else {
				$influencer_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$influencer_recommended_plugin['button_class'] = 'install-now button';
					$influencer_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$influencer_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$influencer_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$influencer_recommended_plugin['button_class'] = 'deactivate-now button';
					$influencer_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$influencer_recommended_plugin['id'] = $slug;
			$influencer_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$influencer_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$influencer_recommended_plugin['description'] = $info->short_description;
			}

			$influencer_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $influencer_recommended_plugin;
