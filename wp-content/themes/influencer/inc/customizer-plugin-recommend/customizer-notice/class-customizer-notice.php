<?php
/**
 * Influencer Customizer Notification System.
 *
 * @package Influencer
 */
 ...
 */
class Influencer_Customizer_Notice {
	...
	/**
	 * The single instance of Influencer_Customizer_Notice
	 *
	 * @var Influencer_Customizer_Notice $instance The Influencer_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Influencer_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Influencer_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Influencer_Customizer_Notice ) ) {
			self::$instance = new Influencer_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $influencer_recommended_plugins;
		...
		$influencer_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$influencer_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'influencer-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', array(), INFLUENCER_THEME_VERSION );
		...
		wp_enqueue_script( 'influencer-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'customize-controls' ), INFLUENCER_THEME_VERSION );
		wp_localize_script(
			'influencer-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => __( 'Activating', 'influencer' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Influencer_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Influencer_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Influencer_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'influencer_show_recommended_plugins' );
			...
			update_option( 'influencer_show_recommended_plugins', $show_recommended_plugins );
		}
